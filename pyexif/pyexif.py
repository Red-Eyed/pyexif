...
class ExifEditor(object):
    def __init__(
        ...
        self._exiftool_path = exiftool_path.resolve().as_posix()
        ...
        self._rotations = {
            ...
            7: (270, 1),
            8: (270, 0)}
        self._invertedRotations = dict(
            [[v, k] for k, v in self._rotations.items()])
        # DateTime patterns
        self._datePattern = re.compile(r"\d{4}:[01]\d:[0-3]\d$")
        self._dateTimePattern = re.compile(
    ...
    def setOrientation(self, val):
        """Orientation codes:
            ...
            8: -90    Normal
        """
        cmd = f"""{self._exiftool_path} {self._optExpr} -Orientation#='{val}' "{self.photo}" """.format(
            **locals())
    ...
    def addKeywords(self, kws):
        ...
        """
        kws = [
            "-iptc:keywords+={0}".format(kw.replace(" ", r"\ ").replace("&", r"\&")) for kw in kws]
        kwopt = " ".join(kws)
        cmd = f"""{self._exiftool_path} {self._optExpr} {kwopt} "{self.photo}" """.format(
            **locals())
        _runproc(cmd, self.photo)
    ...
    def getTag(self, tag, default=None):
        ...
        """
        cmd = f"""{self._exiftool_path} -j -d "%Y:%m:%d %H:%M:%S" -{tag} "{self.photo}" """.format(
            **locals())
    ...
    def getTags(self, just_names=False, include_empty=True):
        ...
        cmd = f"""{self._exiftool_path} -j -d "%Y:%m:%d %H:%M:%S" "{self.photo}" """.format(
            **locals())
        out = _runproc(cmd, self.photo)
        info = json.loads(out)[0]
        if include_empty:
    ...
    def setTag(self, tag, val):
        ...
        valstr = " ".join(vallist)
        cmd = f"""{self._exiftool_path} {self._optExpr} {valstr} "{self.photo}" """.format(
            **locals())
        try:
            out = _runproc(cmd, self.photo)
...
#             dtstring = self._formatDateTime(dttm)
    ...
    def _setDateTimeField(self, fld, dttm):
        ...
        if isinstance(dttm, (datetime.datetime, datetime.date)):
            ...
            dtstring = self._formatDateTime(dttm)
        cmd = f"""{self._exiftool_path} {self._optExpr} -{fld}='{dtstring}' "{self.photo}" """.format(
            **locals())
        _runproc(cmd, self.photo)
    ...
    def _formatDateTime(self, dt):
        ...
        if self._datePattern.match(dt):
            ...
            raise ValueError(
                "Incorrect datetime value '{0}' received".format(dt))
